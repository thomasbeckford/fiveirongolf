...
export function FeaturesSection({ location }: { location: Location }) {
  const { FeaturesSchema } = location;

  const features = FeaturesSchema?.features || [];
            ...
            {features.map((feature, index) => (
              <div
                ...
                className={`
                  ...
                  ${index < features.length - 1 ? 'md:border-r-2 border-border' : ''}
                `}
              >
                {/* Neon Title */}
                <div className="mb-8">
